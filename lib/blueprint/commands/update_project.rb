...
module Blueprint
  module Commands
    class UpdateProject < Base
      ...
      def sass_load_paths
        @sass_load_paths ||= [project_src_directory, blueprint_src_directory]
      end

      # The directory where the blueprint source files are located.
      def blueprint_src_directory
        @blueprint_src_directory ||= File.expand_path(File.join(File.dirname(__FILE__), separate('../../../src')))
      end
